...
# Get evil
EVIL_LINK=https://bitbucket.org/lyro/evil
EVIL_FOLDER=$EMACS_PREFIX/evil
cd $EMACS_PREFIX
if [ ! -d $EVIL_FOLDER ]; then
    $HG clone $EVIL_LINK
fi
cd $EVIL_FOLDER
git pull
...
# Get evil-matchit
EVIL_MATCHIT_GIT=https://github.com/redguardtoo/evil-matchit.git
EVIL_MATCHIT_FOLDER=$EMACS_PREFIX/evil-matchit
cd $EMACS_PREFIX
if [ ! -d $EVIL_MATCHIT_FOLDER ]; then
    git clone $EVIL_MATCHIT_GIT
fi
cd $EVIL_MATCHIT_FOLDER
git pull

# rainbow-delimiters
RAINBOW_DELIMITERS_GIT=https://github.com/Fanael/rainbow-delimiters.git
RAINBOW_DELIMITERS_FOLDER=$EMACS_PREFIX/rainbow-delimiters

cd $EMACS_PREFIX
if [ ! -d $RAINBOW_DELIMITERS_FOLDER ]; then
    $GIT clone $RAINBOW_DELIMITERS_GIT
fi
cd $RAINBOW_DELIMITERS_FOLDER
$GIT pull

# undo-tree
UNDOTREE_GIT=http://www.dr-qubit.org/git/undo-tree.git
UNDOTREE_FOLDER=$EMACS_PREFIX/undo-tree
cd $EMACS_PREFIX
if [ ! -d $UNDOTREE_FOLDER ]; then
    $GIT clone $UNDOTREE_GIT
fi
cd $UNDOTREE_FOLDER
$GIT pull
